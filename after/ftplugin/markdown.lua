...
vim.api.nvim_buf_set_keymap(0, 'n', 'grr', '', {
    ...
    callback = function()
        ...
        local def = GetLink()
        if def then
            if def:match("^Definition%s+%((.-)%)") or def:match("^Theorem%s+%((.-)%)") or def:match("^Corollary%s+%((.-)%)") or def:match("^Lemma%s+%((.-)%)") or def:match("^Proposition%s+%((.-)%)") or def:match("^Example%s+%((.-)%)") or def:match("^Problem%s+%((.-)%)") then
                def = def:match("%((.-)%)")
                fzf.grep({
                    prompt  = "Rg❯ ",
                    search  = "**" .. def .. "**",
